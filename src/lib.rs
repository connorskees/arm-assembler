pub use ast::*;
use decode::InstructionDecoder;
use encode::InstructionEncoder;
...
impl Assembler {
    ...
    // pub fn movz32(&mut self, imm: u16, register: Register32) {
    //     self.instructions.push(Instruction::new(
    //         OpCode::Movz32,
    //         InstructionKind::DataProcessingImmediate,
    //         Operands::two(
    //             Operand::Imm16(imm),
    //             Operand::Register(register.as_register()),
    //         ),
    //     ))
    // }
    ...
    //     self.instructions.push(Instruction::new(
    //         OpCode::Ret,
    //         InstructionKind::Branching,
    //         Operands::zero(),
    //     ));
    // }

    // pub fn ret_reg(&mut self, register: Register) {
    //     self.instructions.push(Instruction::new(
    //         OpCode::Ret,
    //         InstructionKind::Branching,
    //         Operands::one(Operand::Register(register)),
    //     ));
    // }

    // pub fn orr_shifted_register_64(
    //     &mut self,
    //     shift: Shift,
    //     rm: Register,
    //     imm6: u8,
    ...
    //     rd: Register,
    // ) {
    //     assert_eq!(imm6, imm6 & 0b111111);

    //     self.instructions.push(Instruction::new_with_shift(
    //         OpCode::OrrShiftedRegister64,
    ...
    // pub fn mov_register(&mut self, rm: Register, rd: Register) {
    //     self.orr_shifted_register_64(Shift::Lsl, rm, 0, Register::Xzr, rd);
    // }

    // // pub fn str_imm64(&mut self)

    // pub fn encode(&self) -> Vec<u8> {
    //     let mut buffer = Vec::with_capacity(self.instructions.len() * 4);
...
impl InstructionKind {
    pub fn from_integer(n: u32) -> InstructionKind {
        match (n >> 25) & 0b1111 {
            0b0000 => InstructionKind::Reserved,
            0b0001 => InstructionKind::Unallocated,
            0b0010 => InstructionKind::ScalableVectorExtensions,
            0b0011 => InstructionKind::Unallocated,
            0b1000 | 0b1001 => InstructionKind::DataProcessingImmediate,
            0b1010 | 0b1011 => InstructionKind::Branching,
            0b0100 | 0b0110 | 0b1100 | 0b1110 => InstructionKind::LoadsAndStores,
            0b0101 | 0b1101 => InstructionKind::DataProcessingRegister,
            0b0111 | 0b1111 => InstructionKind::DataProcessingRegister,
            0b10000..=u32::MAX => unreachable!(),
        }
    }
}
...
impl Operand {
    ...
    pub fn expect_register(self) -> Register {
        match self {
...
pub struct Operands([Operand; 4]);

impl Operands {
    pub const fn get(&self, n: u8) -> Operand {
        self.0[n as usize]
    }
    ...
    pub const fn one(one: Operand) -> Self {
        Self([one, Operand::None, Operand::None, Operand::None])
    }

    pub const fn two(one: Operand, two: Operand) -> Self {
        Self([one, two, Operand::None, Operand::None])
    }

    pub const fn three(one: Operand, two: Operand, three: Operand) -> Self {
        Self([one, two, three, Operand::None])
    }

    pub const fn four(one: Operand, two: Operand, three: Operand, four: Operand) -> Self {
...
impl Instruction {
    pub const fn new(op_code: OpCode, kind: InstructionKind, operands: Operands) -> Self {
        Self {
            op_code,
            kind,
            operands,
    ...
    pub fn encode(self, buffer: &mut Vec<u8>) {
        InstructionEncoder::encode(self, buffer)
    }
}
...
mod test {
    ...
    use crate::{asl, Assembler, CaseVisitor, Instruction, InstructionKind, OpCode, Register};

    #[test]
    fn test_parser() {
        let input = crate::preprocess_str(
            r#"
__decode A64
    // A64
    case (29 +: 3, 24 +: 5, 0 +: 24) of
        ...
        let expr = asl::CaseParser::new().parse(&input).unwrap();
        ...
        // println!(
        //     "{}",
        let case = CaseVisitor::write(expr, 0, false, &mut opcodes).unwrap();
        ...
        writeln!(file, "pub enum OpCode {{").unwrap();

        for code in opcodes {
            writeln!(file, "    {code},").unwrap();
        }
        writeln!(file, "    Unallocated,").unwrap();
        writeln!(file, "    Unpredictable,").unwrap();

        writeln!(file, "}}").unwrap();

        writeln!(file, "pub fn foo(inst: u32) -> OpCode {{").unwrap();
        writeln!(file, "{}\n}}", case).unwrap();

        // dbg!(crate::foo::foo(0b11010110000111110000000001100000_u32));

        // write!(file, "{}", opcodes.into_iter().)
        // );
    ...
    fn br32() {
        // br	x3
        let inst = Instruction::decode(0b11010110000111110000000001100000_u32);

        assert_eq!(inst.kind, InstructionKind::Branching);
        assert_eq!(inst.op_code, OpCode::Br);
    }

    #[test]
    fn mov_imm32() {
        // mov	w8, #19
        let inst_bytes = 0b0_10_100101_00_0000000000010011_01000_u32;
        // let inst_bytes = 0b01010010100000000000001001101000_u32;

        let inst = Instruction::decode(inst_bytes);

        assert_eq!(inst.kind, InstructionKind::DataProcessingImmediate);
        assert_eq!(inst.op_code, OpCode::Movz32);

        // let mut inst_buffer = Vec::new();

        // inst.encode(&mut inst_buffer);

        // assert_eq!(inst_bytes.to_le_bytes().as_slice(), &inst_buffer);
    }

    #[test]
    fn mov_imm64() {
        // mov x0, #0
        let inst = Instruction::decode(0b11010010100000000000000000000000_u32);

        assert_eq!(inst.kind, InstructionKind::DataProcessingImmediate);
        assert_eq!(inst.op_code, OpCode::Movz64);
    }

    #[test]
    fn mov_reg() {
        // mov	x0, x22
        let inst_bytes = 0b10101010000101100000001111100000_u32;

        let inst = Instruction::decode(inst_bytes);

        assert_eq!(inst.kind, InstructionKind::DataProcessingRegister);
        assert_eq!(inst.op_code, OpCode::OrrShiftedRegister64);

        let mut assembler = Assembler::new();
        assembler.mov_register(Register::X22, Register::X0);

        assert_eq!(inst_bytes.to_le_bytes().as_slice(), &assembler.encode());
    }

    #[test]
    fn str_reg() {
        // str	x9, [sp, #80]
        let inst_bytes = 0b11111001000000000010101111101001_u32;

        let inst = Instruction::decode(inst_bytes);

        assert_eq!(inst.kind, InstructionKind::LoadsAndStores);
        assert_eq!(inst.op_code, OpCode::StrImm64);
    ...
    fn ret() {
        // ret
        let inst_bytes = 0b11010110010111110000001111000000_u32;

        let inst = Instruction::decode(inst_bytes);

        assert_eq!(inst.kind, InstructionKind::Branching);
        assert_eq!(inst.op_code, OpCode::Ret);
